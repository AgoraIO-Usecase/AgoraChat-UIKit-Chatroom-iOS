...
final class UIComponentsExampleViewController: UIViewController {
    ...
    //Gift message display area upon message list.
    ...
    lazy var inputBar: MessageInputBar = {
        MessageInputBar(frame: CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: 52),text: nil,placeHolder: "Let's chat!.")
    }()
    //Gift page 1
    ...
    override func viewDidLoad() {
        ...
        //View global notify
        let button = UIButton(type: .custom).frame(CGRect(x: 100, y: 140, width: 150, height: 20)).textColor(.white, .normal).backgroundColor(UIColor.theme.primaryColor6).cornerRadius(.extraSmall).title("Global Notify", .normal).addTargetFor(self, action: #selector(addCarouselTask), for: .touchUpInside)
        self.view.addSubview(button)
        self.carouselTextView.alpha = 0
        
        // Switch ChatmessageDisplayStyle
        let switchCellStyle = UIButton(type: .custom).frame(CGRect(x: 100, y: self.speakerSegment.frame.maxY+5, width: 150, height: 40)).textColor(.white, .normal).backgroundColor(UIColor.theme.primaryColor6).cornerRadius(.small).title(".all", .normal).title("LongPress", .normal).font(.systemFont(ofSize: 16, weight: .semibold))
